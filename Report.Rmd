...
# graph the mean total mozambique cases over time
mozambique_new_cases <- mozambique_owid_data %>%
  group_by(date) %>%
  summarize(new_cases = (new_cases)) %>%
  ggplot(aes(x = lubridate::ymd(date),
             y = new_cases)) +
  geom_line(color = "blue") +
  labs(title = ("New COVID-19 Cases Over-Time in Mozambique"),
       x = "Date",
       y = "Number of new COVID-19 Cases")

mozambique_new_cases
```

**Figure 4a**: Number of newly reported COVID-19 cases in Mozambique.
