...
three_african_nations <- all_owid_data %>%
  dplyr::filter(`location` == "Mozambique" | `location` == "Niger" | `location` == "Malawi") %>% 
  dplyr::filter(`date` == "2021-05-03")

three_african_nations_plot <- ggplot(data = three_african_nations,
                                     aes(x = location,
                                         y = median_age,
                                         fill = location)) +
...
three_other_countries <- all_owid_data %>%
  dplyr::filter(`location` == "United States" | `location` == "United Kingdom" | `location` == "Italy") %>% 
  dplyr::filter(`date` == "2021-05-03")

three_other_countries_plot <- ggplot(data = three_other_countries,
                                     aes(x = location,
                                         y = median_age,
                                         fill = location)) +
  geom_bar(stat='identity') + 
